-- [[ Roblox Modern UI Library - Customizable Version ]]
-- Tính năng: Tùy chỉnh màu sắc, kích thước, transparency
-- Sử dụng độc lập, không cần file khác
...
function UI.CreateWindow(config)
    ...
    local transparency = config.Transparency or 0.4  -- 0-1: 0 = opaque, 1 = transparent
    local cornerRadius = config.CornerRadius or 24
    local toggleIcon = config.ToggleIcon
    local showToggle = config.ShowToggle ~= false  -- mặc định true
    local sidebarWidth = config.SidebarWidth or 100
    local headerHeight = config.HeaderHeight or 80
    
    -- Lấy màu theme
    local ThemeColor = Themes[theme] or Themes.Dark
    config.ThemeColor = ThemeColor  -- lưu lại để dùng sau
    ...
    BorderCorner.CornerRadius = UDim.new(0, cornerRadius + 2)
    ...
    TopSection.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
    TopSection.BackgroundTransparency = transparency
    TopSection.BorderSizePixel = 0
    TopSection.Size = UDim2.new(1, 0, 0, headerHeight)
    TopSection.Parent = MainFrame

    local TopCorner = Instance.new("UICorner")
    TopCorner.CornerRadius = UDim.new(0, cornerRadius)
    ...
    ContentArea.BackgroundTransparency = transparency + 0.1
    ...
    if showToggle then
        ...
        ToggleInner.BackgroundTransparency = 0.2
        ...
        if toggleIcon then
            ...
            ToggleImg.BackgroundTransparency = transparency
            ...
            ToggleText.BackgroundTransparency = transparency
    ...
    local Window = {
        ...
        SetTheme = function(self, newTheme)
            local color = Themes[newTheme]
            if not color then return end
            self.ThemeColor = color
            ThemeColor = color
            
            -- Cập nhật các thành phần
            BorderGlow.BackgroundColor3 = color
            Glow.ImageColor3 = color
            DiagonalBar.BackgroundColor3 = color
            TitleLabel.TextColor3 = color
            
            if ToggleBtn then
                ToggleBtn:FindFirstChild("Frame").BackgroundColor3 = color
                local btn = ToggleBtn:FindFirstChildOfClass("ImageButton") or ToggleBtn:FindFirstChildOfClass("TextButton")
                if btn then
            ...
            for _, tab in pairs(self.Tabs) do
                if tab.Button and tab.Button:FindFirstChild("Frame") and tab.Button:FindFirstChild("Frame").Visible then
                    tab.Button:FindFirstChild("Frame").BackgroundColor3 = color
                end
                if tab.Button and tab.Button:FindFirstChild("ImageLabel") then
                    -- Cập nhật icon color nếu tab đang active
                end
            end
        ...
        -- Hàm đổi transparency
        SetTransparency = function(self, value)
            local t = math.clamp(value, 0, 0.9)
            MainFrame.BackgroundTransparency = t
            TopSection.BackgroundTransparency = t
            Sidebar.BackgroundTransparency = t
            ContentArea.BackgroundTransparency = t + 0.1
            if ToggleBtn then
                ToggleBtn.BackgroundTransparency = t
                local inner = ToggleBtn:FindFirstChild("Frame")
                if inner then
                    local child = inner:FindFirstChildOfClass("ImageButton") or inner:FindFirstChildOfClass("TextButton")
                    if child then
                        child.BackgroundTransparency = t
                    end
                end
            end
            -- Cập nhật các section con
            for _, tab in pairs(self.Tabs) do
                if tab.Content then
                    for _, child in ipairs(tab.Content:GetDescendants()) do
                        if child:IsA("Frame") or child:IsA("TextButton") then
                            if child.BackgroundTransparency ~= nil then
                                -- Giữ nguyên các thành phần có transparency đặc biệt
                            end
                        end
                    end
                end
        ...
        SetSize = function(self, newWidth, newHeight)
            local w = newWidth or width
            local h = newHeight or height
            width = w
            height = h
    ...
    function Window:AddTab(cfg)
        ...
        TabBtn.BackgroundTransparency = transparency
        ...
        TabIconLabel.Position = UDim2.new(0.5, -18, 0.5, -18)
        TabIconLabel.Size = UDim2.new(0, 36, 0, 36)
        TabIconLabel.Image = GetIcon(tabIcon)
        TabIconLabel.ImageColor3 = Color3.fromRGB(180, 180, 180)
        TabIconLabel.Parent = TabBtn

        -- Tooltip
        local Tooltip = Instance.new("TextLabel")
        Tooltip.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
        Tooltip.BackgroundTransparency = transparency
        ...
        TabBtn.MouseButton1Click:Connect(function()
            for _, t in pairs(Window.Tabs) do
                ...
                if img then img.ImageColor3 = Color3.fromRGB(180, 180, 180) end
            end
            TabContent.Visible = true
            Indicator.Visible = true
            TweenService:Create(Indicator, TweenInfo.new(0.25), {Size = UDim2.new(0, 4, 1, 0)}):Play()
            TabIconLabel.ImageColor3 = ThemeColor
        end)
        ...
        function Tab:AddSection(sname)
            ...
            SectionHead.BackgroundTransparency = transparency
            ...
            function Section:AddToggle(tname, opts)
                ...
                ToggleF.BackgroundTransparency = transparency + 0.1
                ...
                ToggleS.BackgroundTransparency = transparency
            ...
            function Section:AddSlider(slname, opts)
                ...
                SliderF.BackgroundTransparency = transparency + 0.1
                ...
                InputB.BackgroundTransparency = transparency
                ...
                SliderB.BackgroundTransparency = transparency
            ...
            function Section:AddDropdown(dname, opts)
                ...
                DropF.BackgroundTransparency = transparency + 0.1
                ...
                for i, item in ipairs(items) do
                    ...
                    ItemBtn.BackgroundTransparency = transparency
            ...
            function Section:AddTextBox(tbname, opts)
                ...
                TBF.BackgroundTransparency = transparency + 0.1
