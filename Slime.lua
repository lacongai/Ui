...
function Window:_Build()
	...
	-- Nút thu nhỏ / phóng to (Minimize) — thu cửa sổ về chỉ còn header
	...
	minBtn.MouseButton1Click:Connect(function()
		...
		if minimized then
			tween(outer, {Size = UDim2.new(0, self.Width, 0, 46)}, 0.22, Enum.EasingStyle.Quint)
		else
			tween(outer, {Size = UDim2.new(0, self.Width, 0, self.Height)}, 0.22, Enum.EasingStyle.Quint)
		end
	end)
	self._minimized = function() return minimized end

	-- Tay cầm Resize — kéo tự do ở góc dưới-phải để đổi Width/Height
	...
	for i = 0, 2 do
		for j = 0, (2 - i) do
			...
			grip.Size = UDim2.new(0, 3, 0, 3)
			grip.Position = UDim2.new(1, -6 - i * 6, 1, -6 - j * 6)
	...
	UserInputService.InputChanged:Connect(function(input)
		if resizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			local newWidth = math.max(360, resizeStartSize.X.Offset + delta.X)
			local newHeight = math.max(240, resizeStartSize.Y.Offset + delta.Y)
...
function Window:SetSize(width, height)
	...
	if self._minimized and self._minimized() then
		self.Outer.Size = UDim2.new(0, width, 0, 46)
	else
		self.Outer.Size = UDim2.new(0, width, 0, height)
	end
end
...
function Window:AddTab(cfg)
	...
	-- Tính chiều rộng nút thủ công bằng TextService (KHÔNG dùng
	-- AutomaticSize lồng 2 lớp — nguyên nhân gây lỗi hiển thị tương tự
	-- lỗi đã gặp ở Section) để chắc chắn nút luôn đủ rộng chứa chữ
	local textWidth = TextService:GetTextSize(name, 13, Enum.Font.GothamSemibold, Vector2.new(1000, 50)).X
	tabButton.Size = UDim2.new(0, textOffset + textWidth + 14, 1, 0)
...
function Tab:AddSection(name)
	...
	sectionFrame.Size = UDim2.new(1, 0, 0, 40) -- sẽ được tính lại thủ công ngay bên dưới
	...
	-- Tự tính chiều cao thủ công (KHÔNG dùng AutomaticSize lồng nhau)
	-- mỗi khi có thêm Button/Toggle/Slider/... vào body
	local BOTTOM_PAD = 12
	local function updateHeight()
		local h = bodyLayout.AbsoluteContentSize.Y
		body.Size = UDim2.new(1, -24, 0, h)
		sectionFrame.Size = UDim2.new(1, 0, 0, 34 + h + BOTTOM_PAD)
	end
	bodyLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateHeight)
	updateHeight()
